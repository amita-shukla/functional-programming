...
class FunSetSuite extends FunSuite {
  ...
  test("intersect") {
    new TestSets {
      ...
      val s_2 = intersect(s1, s4)
      assert(!contains(s, 1) && !contains(s, 2))
      assert(contains(s_2, 1))
    }
  }
  ...
  test("diff") {
    new TestSets {
      ...
      val s_2 = union(s, s3)
      //1,2,3
      val s_3 = diff(s, s4) //2,3
      assert(!contains(s_3, 1))
    }
  }
  ...
  test("forall") {
    new TestSets {
      val S = union(union(s1, s2), s3)
      //1,2,3
      val S1 = singletonSet(1001)
      assert(forall(S, x => x < 4))
      assert(forall(S, x => x > 0))
      assert(!forall(S1, x => x == 1001))
    }
  }
